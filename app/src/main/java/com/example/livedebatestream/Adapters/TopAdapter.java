...
public class TopAdapter extends RecyclerView.Adapter<TopAdapter.MyViewHolder> {
    ...
    public TopAdapter(List<Topusers>topList){
        this.topList=topList;
    }
    @Override
    public TopAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.top_users,parent,false);
        return new MyViewHolder(itemView);
    }
    @Override
    public void onBindViewHolder(MyViewHolder holder, int position) {
        Topusers row=topList.get(position);
